package jwtm.clusteringengine.model.duplicate;
		...
		for (int i=0;i<numDocs;i++){
			if(duplicateList[i]==1){
				if (verbose){
					...
					System.out.println("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
					System.out.println("DocID: \""+i+"\" | "+similarityMatrix.getIndexReader().document(i).get("id"));
				}
				for (int j=0;j<numDocs;j++){
					if (duplicateMatrix[i][j]>0){
						if (verbose){
							System.out.println("DocID: \""+j+"\" | "+similarityMatrix.getIndexReader().document(j).get("id")+ " | cosSimAll: "+simMatrix[i][j]+" | cosSimTitle: "+simMatrixTitle[i][j]+" | cosSimDesc: "+simMatrixDesc[i][j]);
						}
						clusterCount[i]++;
